...
NOT YET CALIBRATED against a real statement — same situation icici.py started
in. Run `debug-extract` on a real IndusInd message, inspect the local-only
output file, and correct these patterns to match. Returns None/low-confidence
rather than guessed values until calibrated (see icici.py's module docstring
for why this matters and how the calibration workflow works).
"""
...
_DUE_DATE_RE = re.compile(r"(?:Payment\s+)?Due\s+Date\s*[:\-]?\s*(\d{1,2}[/\-]\w+[/\-]\d{2,4})", re.IGNORECASE)
...
class IndusindStatementParser(StatementParser):
    ...
    def extract_statement_metadata(self, pdf_text: str) -> ExtractedStatement:
        ...
        found_fields = [total_amount_due, minimum_amount_due, card_match]
